...
class QuizComponent extends Component {
  ...
  fetchQuestions = async () => {
    ...
    const questionBank = [];
    Object.keys(snapshotVal).forEach(key => questionBank.push({...snapshotVal[key], id: key}));

    const questions = [];
    const selected = {};
    for (let i = 0; i < numQuestions; i++) {
      let randomIndex = this.getRandomIndex(questionBank);
      while (selected[randomIndex]) randomIndex = this.getRandomIndex(questionBank);
  ...
  handleInvitationClick = async () => {
    ...
    db.ref('games').push(initialGameData).then(docRef => {
              ...
              <Container maxWidth="sm">
                ...
                <div className={classes.heroButtons}>
                  <Grid container spacing={2} justify="center">
                      <Grid item>
                        <Button variant="contained" color="primary" onClick={this.handleInvitationClick}>
                          Invite Friend
                        </Button>
                      </Grid>
                      <Grid item>
